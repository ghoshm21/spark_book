...
def skew_orders_generator(cust_id, skew_date):
    current_year = dt.datetime.today().year
    # get the main data points
    order_id = uuid.uuid4()
    order_date = dt.datetime.strptime(skew_date, '%Y-%m-%d')
    shipping_date = addDay(order_date, randrange(15))
    delivery_date = addDay(shipping_date, randrange(45))
    shipping_company = bus.company()
    return str(order_id),str(cust_id),str(order_date),str(shipping_date),str(delivery_date),shipping_company
...
# skewed the data for order
# generate orders based on cust_id and hard coded order_date to skew the data
for i in range(10):
    order_rdd = customer_df.select("cust_id",lit("2020-01-01")).sample(True, fraction=0.1).rdd.map(lambda x: skew_orders_generator(x[0],x[1]))
    order_df = order_rdd.toDF(schema=order_df_schema)
    order_df.write.option("compression","gzip").save(path=file_dir+'/order_skew', format='csv', mode='append', sep='|')
    print("Order generated for %s" %i)
