...
# ----------------------- Test with order skew data ----------------------------------------------------------
# Load the skew data in to order
orders = spark.read.format('orc').schema(order_df_schema).load(file_dir+'/orders_skw_orc/*.orc')
orders.createOrReplaceTempView("orders")
spark.sql("select cust_id, count(1) as rec_count from orders group by cust_id order by count(1) desc").show(8,False)
+---------------------------------------+---------+
|cust_id                                |rec_count|
+---------------------------------------+---------+
|308866526326142099064477198020167058098|200000001|
|307643218223138015641709187086459060914|200000001|
|300027649736679849154714606614789472946|200000001|
|310487679122580535944057215461750981298|200000001|
|32418570489867457558547660609317749426 |23       |
|256346305982744030324735983239333921458|23       |
|254293191103066977821570918218611278514|23       |
|46140677648871197582688745161315566258 |23       |
